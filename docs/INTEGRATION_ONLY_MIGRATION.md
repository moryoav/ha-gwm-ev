...
Status: `v0.16.10` completed the tester's current-app password and one-time-code flow, published the session, created the Home Assistant entry, and completed vehicle discovery. The first `getLastStatus` read then returned an application error whose sanitized code was lost at the coordinator boundary. Release `v0.16.11` exposes that code for the next stored-session retry without requiring another verification code.
...
- Exact synthetic wire fixtures and Home Assistant tests cover both route families, fixed path-signature vectors, empty query values, new-entry defaults, existing-entry compatibility, reauthentication, reconfiguration, restart persistence, reads, commands, result polling, state retirement, and the no-fallback boundary. The direct gateway's signing acceptance is live-proven with a nonexistent account. The tester has now live-proven current password authentication, verification delivery and acceptance, session publication, config-entry creation, and vehicle discovery; status reading remains at the narrowed request-contract investigation stage.
