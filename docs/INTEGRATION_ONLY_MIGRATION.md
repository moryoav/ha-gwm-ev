...
| D-071 | 2026-08-31 | I use the path as the current ANZ signing target for direct gateway requests. | The tester's `v0.16.8` attempt returned exact `607099`. Two controlled live requests with the same nonexistent `.invalid` account then isolated the variable: full-URL signing returned `607099` (`sign is inconformity`), while path signing reached account validation and returned `308001`. Release `v0.16.9` applies that evidence to authentication, reads, commands, and result polling, and classifies any exact authentication `607099` as a secret-safe signature error. |
...
Status: `v0.16.10` completed the tester's current-app password and one-time-code flow, published the session, created the Home Assistant entry, and completed vehicle discovery. Release `v0.16.11` exposed the first `getLastStatus` failure as exact API code `607099`. The signed app's split networking architecture explains the failure: current authentication is handled by Flutter, while vehicle reads use the native request interceptor. Release `v0.16.12` applies the native read signing policy while retaining the proven current session and headers. Live confirmation is pending.

Delivered:

- I inspected the signed GWM ANZ 1.0.6 application outside the repository and recovered its current v2 password-login and verification route family, request fields, regional signing profile, and ordinary TLS behavior. Static analysis initially suggested a full absolute signing target, but controlled direct-gateway probes proved that this client must sign the path. The distinct decoded-query policy remains unchanged.
...
- I keep current v2 login, code request, and code check isolated from the legacy v1 endpoints. Current requests use the live-verified path target with compact JSON, 32-character nonce, URI encoding, exact content type, full device ID, current headers, and email-or-phone account type. The unavailable native push token is represented by the app's nullable field.
- I apply the signed app's current password input rules before constructing a v2 login request: only its allowed character class survives, ASCII spaces are removed, and the result is limited to 40 characters. The legacy request keeps the supplied password unchanged.
...
- I persist the selected method in the config entry, client configuration, authentication state, and durable authentication context. Restarted polling, commands, and result polling therefore retain the selected current session and headers instead of silently returning to legacy authentication. Vehicle reads use the official app's separate native request signing policy. Existing states without the method decode as legacy, and changing the method retires the previous account-bound session before another login.
...
- Exact synthetic wire fixtures and Home Assistant tests cover both route families, fixed path-signature vectors, the split authentication and native-read signing policies, empty query handling, new-entry defaults, existing-entry compatibility, reauthentication, reconfiguration, restart persistence, reads, commands, result polling, state retirement, and the no-fallback boundary. The direct gateway's authentication signing acceptance is live-proven with a nonexistent account. The tester has now live-proven current password authentication, verification delivery and acceptance, session publication, config-entry creation, and vehicle discovery. Exact `607099` at the first query-bearing status request identified the native-read signing mismatch; live confirmation of the `v0.16.12` correction is pending.
...
- Exact live parity of undocumented authentication and response behavior in ANZ and Russia remains incomplete. EU authentication and read behavior are proven live on one account. ANZ current-app `v0.16.10` completed real-account password authentication, one-time verification, session publication, config-entry creation, and vehicle discovery. Release `v0.16.11` then identified exact `607099` at `getLastStatus`, and `v0.16.12` separates the native vehicle-read signer from the proven current authentication signer. Live read confirmation remains pending. ANZ legacy v1 and Task 10 Russia semantics remain fixture-proven until another live test succeeds.
