...
Status: implemented on 2026-08-31 for release `v0.16.7`; current-app login is fixture-proven and awaits the first successful live account test.

Delivered:

- I inspected the signed GWM ANZ 1.0.6 application outside the repository and recovered its current v2 password-login and verification route family, request-field evidence, regional signing profile, and ordinary TLS behavior. I combined that evidence with the established GWM v2 request shape for this first beta.
...
- I keep current v2 login, code request, and code check isolated from the legacy v1 endpoints. Accepted sessions continue through the existing ANZ profile, refresh, read, and command contracts.
- I persist the selected method in the config entry and durable authentication context. Existing entries that do not contain the field remain byte-compatible with their legacy context, while changing the method retires the previous account-bound session before another login.
...
- Exact synthetic wire fixtures and Home Assistant tests cover both route families, new-entry defaults, existing-entry compatibility, reauthentication, reconfiguration, restart persistence, state retirement, and the no-fallback boundary. Live v2 response and verification behavior still depends on tester logs.
...
- Exact live parity of undocumented authentication and response behavior in ANZ and Russia remains unverified. EU authentication and read behavior are proven live on one account. ANZ legacy v1, current-app v2, and Task 10 Russia semantics remain fixture-proven only.
...
- Legacy ANZ `309702`/`110641` and current-app `308103`/`110641` challenges, current token-expiry and rotation behavior, verification delivery and expiry, AU-versus-NZ differences, and unknown code-check failures lack sanitized current-service evidence. Unknown errors stop without attempting a second method or final login.
