...
Status: `v0.16.8` reached the intended current-app login endpoint but returned exact `607099`. Controlled live synthetic probes proved that the direct gateway rejects full-URL signing and accepts path signing. Release `v0.16.9` applies the isolated correction and awaits the tester's account flow.
