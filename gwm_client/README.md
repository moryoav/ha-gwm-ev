...
For BeanTech `horn`, `flash_lights`, and `horn_and_lights`, I use the PIN-exempt v3 timely endpoint. When polling these commands with `ChinaClient.get_remote_command_results`, pass the original action as `control_action` to select its v3 result stream. Omit this argument for existing legacy commands. Home Assistant retains the action through its command journal so polling continues after a restart. Command submission does not automatically retry or fall back to another endpoint.
